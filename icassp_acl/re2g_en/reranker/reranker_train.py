...
#                 correctness_batch += correctness

#                 if (line_ndx + 1) % self.args.instances_size == 0:
#                     logits = self.one_batch(query_batch, passages_batch)
#                     nll = -(logits.dot(torch.tensor(correctness_batch).to(logits.device)))
