...
from transformers import (BertConfig, BertTokenizer, BertForSequenceClassification, RobertaConfig, RobertaForSequenceClassification, RobertaTokenizer)
import random
...
class RetrieverDPRReranker(RetrieverBase):
    def __init__(self, hypers: RerankerHypers, dpr_hypers: DPRHypers, bm25_hypers: BM25Hypers, *, apply_mode=False):
        ...
        #                                         num_labels=2)
        model, self.tokenizer = load_pretrained(hypers, RobertaConfig, RobertaForSequenceClassification, RobertaTokenizer,
                                        num_labels=2)
        if apply_mode:
            self.model = model
            self.optimizer = None
    ...
    def retrieve_forward(self, queries, *, positive_pids=None, rdocs=None):
        ...
        if self.bm25_retriever is not None:
            # MARK
            title_text_dpr = [set([text for title, text in zip(doc_dpr['title'], doc_dpr['text'])])
