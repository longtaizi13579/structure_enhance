...
class CorpusClient:
        ...
        if not hasattr(hypers, 'global_rank') or hypers.global_rank == 0:
            ...
            while True:
                ...
                try:
                    ...
                    logger.warning(f'Waiting on corpus server to start: {print(hypers.corpus_endpoint + "/config")}')
                    continue
    ...
    def string_retrieve(self, question_encoder, query_strs, *, n_docs=5, return_scores=False):
        input_dict = prepare_seq2seq_batch(self.tokenizer, query_strs, return_tensors="pt", max_length=512)
