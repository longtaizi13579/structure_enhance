...
from transformers import (DPRQuestionEncoder, DPRContextEncoder, BertModel, DPRConfig)
import os
...
class BiEncoder(torch.nn.Module):
    ...
    def __init__(self, hypers: BiEncoderHypers):
        ...
        if hypers.qry_encoder_name_or_path == 'hfl/chinese-roberta-wwm-ext':
            bert_model = BertModel.from_pretrained('hfl/chinese-roberta-wwm-ext')
            config = DPRConfig()
            config.vocab_size = bert_model.config.vocab_size
            question_encoder = DPRQuestionEncoder(config)
            question_encoder.base_model.base_model.load_state_dict(bert_model.state_dict(), strict=True)
        else:
            question_encoder = DPRQuestionEncoder.from_pretrained(hypers.qry_encoder_name_or_path)
        self.qry_model = EncoderWrapper(question_encoder)
        
        
        if hypers.ctx_encoder_name_or_path == 'hfl/chinese-roberta-wwm-ext':
            bert_model = BertModel.from_pretrained('hfl/chinese-roberta-wwm-ext')
            config = DPRConfig()
            config.vocab_size = bert_model.config.vocab_size
            context_encoder = DPRContextEncoder(config)
            context_encoder.base_model.base_model.load_state_dict(bert_model.state_dict(), strict=True)
        else:
            context_encoder = DPRContextEncoder.from_pretrained(hypers.ctx_encoder_name_or_path)
        self.ctx_model = EncoderWrapper(context_encoder)
