...
from my_model import GraphEncoder
...
logging.basicConfig(filename='512_origin_train_graph_without_structure',level=logging.DEBUG)
...
save_base_directory = './512_origin_train_graph_without_structure_file/'
...
def load_data(file_addr):
    ...
    for every_file_addr in concrete_addr:
        ...
        for every_dialog in dialogs:
            ...
            get_negatives = re.split('<1> |<2> |<3> ', now_negative['text'].strip())
            get_query = re.split('<1> |<2> |<3> ', every_dialog['question'])
            get_answer = re.split('<1> |<2> |<3> ', every_dialog['positive_ctxs'][0])
            all_negatives.append(get_negatives[-1])
            all_answer.append(get_answer[-1])
            all_query.append(get_query[-1])
            ...
            for every_node in graph_nodes['node']:
                get_node_text = re.split('<1> |<2> |<3> ', every_node['text'])
                all_node_text.append(get_node_text[-1])
...
for epoch in range(num_epochs):
    ...
    for index, sample in enumerate(train_dataloader):
        ...
        for every_graph in sample['graph_content']:
            one_graph = ctx_tokenizer(every_graph, padding=True, truncation=True, max_length=512, return_tensors='pt')
            one_graph = {k: v.cuda() for k, v in one_graph.items()}
            whole_graph.append(one_graph)
        ctx = ctx_tokenizer(sample['answer'], padding=True, truncation=True, max_length=512, return_tensors='pt') # max 256
