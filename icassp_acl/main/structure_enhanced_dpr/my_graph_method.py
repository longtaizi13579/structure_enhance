...
class myRGCNConv(MessagePassing):
    ...
    def __init__(
            ...
            bias: bool = True,
            root_trans: bool = True,
            edge_trans: bool = True,
            **kwargs,
    ):
    ...
    def forward(self, x: Union[OptTensor, Tuple[OptTensor, Tensor]],
                edge_index: Adj, edge_type: OptTensor = None, gate_weight: OptTensor = None):
        r"""
        Args:
        ...
        if self.num_blocks is not None:  # Block-diagonal-decomposition =====
            ...
            for i in range(self.num_relations):
                ...
                tmp_gate_weight = masked_edge_weight(gate_weight, edge_type == i)
                h = self.propagate(tmp, x=x_l, edge_type_ptr=None, size=size, gate_weight=tmp_gate_weight)
            ...
            if (self._WITH_PYG_LIB and self.num_bases is None
                ...
                out = self.propagate(edge_index, x=x_l,
                                     edge_type_ptr=edge_type_ptr, size=size, gate_weight=gate_weight)
            else:
                for i in range(self.num_relations):
                    tmp = masked_edge_index(edge_index, edge_type == i)
                    tmp_gate_weight = masked_edge_weight(gate_weight, edge_type == i)
                    if x_l.dtype == torch.long:
                        ...
                        if self.edge_trans:
                            h = self.propagate(tmp, x=x_l, edge_type_ptr=None,
                                            size=size, gate_weight=tmp_gate_weight)
                            out = out + (h @ weight[i]) * torch.tensor(0.1).cuda()
                            #print(3)
                        else:
                            out = out + self.propagate(tmp, x=x_l, edge_type_ptr=None,
                                            size=size, gate_weight=tmp_gate_weight) * torch.tensor(0.1).cuda()
                            #print(4)
        root = self.root
        if root is not None:
            if self.root_trans:
                ...
                out = out + x_r * torch.tensor(0.8).cuda()
    ...
    def message(self, x_j: Tensor, edge_type_ptr: OptTensor, gate_weight: OptTensor) -> Tensor:
        # x_j is source vector
        if edge_type_ptr is not None:
            # TODO Re-weight according to edge type degree for `aggr=mean`.
            if gate_weight is not None:
                return segment_matmul(x_j, edge_type_ptr, self.weight) * gate_weight.view(-1, 1)
            return segment_matmul(x_j, edge_type_ptr, self.weight)
        if gate_weight is not None:
            result = x_j * gate_weight.view(-1, 1)
            return result
        #print(5)
        return x_j
