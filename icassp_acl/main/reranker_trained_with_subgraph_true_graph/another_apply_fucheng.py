...
from reranker_trained_with_subgraph_true_graph.another_model import RerankerHypers, load_for_apply
import torch
...
def one_instance(args, model, tokenizer, query, passages, subgraphs, subgraphs_id):
    ...
    for every_cluster in subgraphs:
        ...
        for every_passage in every_cluster:
            if '<3>' in every_passage:
                every_passage = every_passage.replace('<3>', '[unused1]')
            else:
                every_passage = every_passage.replace('<2>', '[unused1]')
    ...
    for every_cluster_index in range(len(subgraphs)):
        ...
        for every_passage_index in range(len(now_cluster)):
            ...
            other_passages = [other_passage.split('[unused1]')[-1] for other_passage in other_passages]
    ...
    texts_a = [query] * (len(texts_c) + len(texts_b))
    passage_length = len(texts_b)
    texts_b.extend(texts_c)
    ...
    passage_logits, subgraph_logits = model(inputs, passage_length)
